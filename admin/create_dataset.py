...
def num_samples(dataset, train):
    if dataset == "diffuse":
        # 1 for testing
        return 1 if train else 2000 # this number is specific to the number of TM examples we have for training
    else:
...
def read_images(folder: str, tag: str) -> np.array:
    """
    ...
    images = []
    # the image names by definition need to correspond to one another for this to work of course
    # images are weird - use numpy arrays but normalise yourself
    for file in os.listdir(directory):
        filename = os.fsdecode(file)
        if filename.endswith(".npy"):
            num = filename.split(".")[0][17:]
            #image0 = np.array(Image.open(folder + "/" + filename))   
            #image1 = np.array(Image.open(folder + "/../target/%s%s.png"%(tag, num)))
            #images.append(np.array(Image.open(filename)))
            image0 = normalise_data(np.load(folder + "/" + filename))
            image1 = normalise_data(np.load(folder + "/../target/%s%s.npy"%(tag, num))) # need to import dFF and SRO separaetly
            images.append(staple_image(image0, image1))

    return np.asarray(images).astype(np.uint8) # add on the channel dimension at the end
...
if __name__ == "__main__":
    ...
    cond_folder_loc = "/data/lrudden/ML-DiffuseReader/dataset/training/conditional" #sys.argv[1]
    target_folder_loc = "/data/lrudden/ML-DiffuseReader/dataset/training/target" #sys.argv[2]

    images_dFF = read_images(cond_folder_loc, "dFF")
    images_SRO = read_images(cond_folder_loc, "SRO")

    store_many_lmdb(images_dFF, "/data/lrudden/ML-DiffuseReader/dataset/training/train_lmdb_dFF")
    store_many_lmdb(images_SRO, "/data/lrudden/ML-DiffuseReader/dataset/training/train_lmdb_SRO")
