...
def _do_move_(accept, reject, move, spins, selected, totsites, bufferSize, supercell, neighbours, neighboursInDirections, targetCorrelations, rng, k, MC_TEMP):
    """
    k is a list of force constants
    Internal parrallelisation function for MC step
    """
    ...
    return supercell, np.sum(deltaE), accept, reject   

def MCOrdering(BOX_SIZE,SITE_ATOMS,VARIANT_ABUNDANCE,MC_CYCLES,MC_TEMP, TARGET_CORRELATIONS, FORCE_CONSTANTS, NEIGHBOUR_DIRECTIONS,supercell,neighbours, modelType, ncpus=4):
    ...
    if modelType == 0: nmoves = int((natoms*ncells)/(sm))
    else: nmoves = 50
        ...
        if not stop:
            ...
            if ncpus < 2:
                for m in range(nmoves):
                    ...
                    supercell, E, acc, rej = _do_move_(accept, reject, move, spins, selected, totsites, bufferSize, supercell, neighbours, neighboursInDirections, targetCorrelations, rng, k, MC_TEMP)
                    accept += acc
                    reject += rej
                    #delt = time.time() - t0
                    #print(">> Time %.2f with delE %.2f"%(delt, E))
            else:
                import concurrent.futures
                #param_list = [move, spins, selected, totsites, bufferSize, supercell, neighbours, neighboursInDirections, targetCorrelations, rng, k, MC_TEMP]
                for m in range(nmoves):
                    move = np.zeros((sm,2),dtype=int)
                    spins = np.zeros((sm,2),dtype=int)
                    selected = np.zeros(len(supercell),dtype=int)
                    with concurrent.futures.ThreadPoolExecutor(max_workers=ncpus) as executor:
                        future = [executor.submit(_do_move_, accept, reject, move, spins, selected, totsites, bufferSize, supercell, neighbours, neighboursInDirections, targetCorrelations, rng, k, MC_TEMP)] 
            ...
            # If this config improves on the previous best one, make a note
                ...
                if checkConvergence(convList,convCrit): 
                    ...
                    if np.any(abs(targetCorrelations - currentCorrelations) > 0.025):continue
                        #start = np.random.randint(0, len(supercell)-len(supercell)/10)
                        #np.random.shuffle(supercell[start:start+int(len(supercell)/10)])
                        #print("shuffled")
                    else:
                        print("\n--------------------------------------------------")
